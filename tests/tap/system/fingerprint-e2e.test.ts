...
test('end-to-end fingerprinting workflow', async (t) => {
  try {
    ...
    t.ok(resultWithoutFingerprints, 'should return result without fingerprints');
    
    if (!legacyPlugin.isMultiResult(resultWithoutFingerprints)) {
      return t.fail('expected multi inspect result');
    }
    
    t.equal(resultWithoutFingerprints.scannedProjects.length, 1, 'should have 1 scanned project');
    const depGraphWithoutFingerprints = resultWithoutFingerprints.scannedProjects[0].depGraph;
    ...
    const dependencyNode = nodes.find(node => node.nodeId !== depGraphJson.graph.rootNodeId);
    
    if (dependencyNode) {
      t.notOk(dependencyNode.info?.labels?.fingerprint, 'should not have fingerprint when disabled');
    }
    ...
    t.ok(resultWithFingerprints, 'should return result with fingerprints enabled');
    
    if (!legacyPlugin.isMultiResult(resultWithFingerprints)) {
      return t.fail('expected multi inspect result with fingerprints');
    }
    
    const depGraphWithFingerprints = resultWithFingerprints.scannedProjects[0].depGraph;
    t.ok(depGraphWithFingerprints, 'should have dependency graph with fingerprints enabled');
    ...
    if (dependencyNodeWithFingerprints) {
      ...
      const hasFingerprint = dependencyNodeWithFingerprints.info?.labels?.fingerprint;
      const hasError = dependencyNodeWithFingerprints.info?.labels?.fingerprintError;
      
      t.ok(hasFingerprint || hasError, 'should have either fingerprint or error when fingerprinting enabled');
    }
