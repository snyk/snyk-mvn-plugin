...
test('parseDependency returns expected object when input has 4 parts', async () => {
  expect(
      parseDependency('com.example:my-app:jar:1.2.3')).toEqual(
      {
        groupId: 'com.example',
        artifactId: 'my-app',
        type: 'jar',
...
test('parseDependency returns expected object when input has 5 parts', async () => {
  expect(
    parseDependency('com.example:my-app:jar:1.2.3:compile')).toEqual(
    {
      groupId: 'com.example',
      artifactId: 'my-app',
      type: 'jar',
...
test('parseDependency returns expected object when input has 6 parts', async () => {
  expect(
    parseDependency('com.example:my-app:jar:jdk8:1.2.3:compile')).toEqual(
    {
      groupId: 'com.example',
      classifier: 'jdk8',
      artifactId: 'my-app',
...
test('parseDependency returns unknown when input has less than 4 parts', async () => {
  expect(
    parseDependency('foo:bar')).toEqual(
    {
      groupId: 'unknown',
      artifactId: 'unknown',
      type: 'unknown',
