...
describe('buildDepGraph - scope labels', () => {
  describe('with showMavenBuildScope enabled', () => {
    test('should add maven:build_scope labels for all standard Maven scopes', () => {
      ...
      expect(compileNode?.info?.labels).toEqual({ 'maven:build_scope': 'compile' });

      const providedNode = nodes.find(n => n.nodeId === 'test:provided-dep:jar:1.0.0:provided');
      expect(providedNode?.info?.labels).toEqual({ 'maven:build_scope': 'provided' });

      const runtimeNode = nodes.find(n => n.nodeId === 'test:runtime-dep:jar:1.0.0:runtime');
      expect(runtimeNode?.info?.labels).toEqual({ 'maven:build_scope': 'runtime' });
      ...
      expect(systemNode?.info?.labels).toEqual({ 'maven:build_scope': 'system' });
    });
    ...
    test('should handle dependencies without explicit scope (default to compile)', () => {
      ...
      expect(noScopeNode?.info?.labels).toEqual({ 'maven:build_scope': 'compile' });
    });
